...
import de.jonathansautter.autooff.R;
		...
		case MotionEvent.ACTION_MOVE:
			if (mUserIsMovingPointer) {
				...
				if (lockAtStart && lockEnabled) {
					...
					/*if (mIsMovingCW && lastCWDistanceFromStart > cwDistanceFromStart) {
						//Toast.makeText(getContext(), "more", Toast.LENGTH_SHORT).show();
						addition += 60;
					} else if (!mIsMovingCW && lastCWDistanceFromStart < cwDistanceFromStart) {
